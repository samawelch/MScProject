...
Chemical stressors were selected across a diverse range of functional groups (\cref{tab:stressors}) in order to assemble a panel that had some but not all mechanisms of action in common amongst the stressors, and that have the potential to occur in mixtures in ecosystems.
...
Section on sample sizes???

Due to a number of issues with isolate growth in liquid media and manual pipetting error of bacterial cultures, sample size varies across both treatments and isolates (\cref{tab:samples})
